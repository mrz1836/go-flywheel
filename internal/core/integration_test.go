...
func (w *pgConcurrentWorker) Work(_ context.Context, _ *Job[pgConcurrentArgs]) (Result, error) {
	w.processed.Add(1)
	return Result{}, nil
}
...
func newPostgresRunner(t *testing.T, db *gorm.DB, reg *Registry, concurrency int) *Runner {
	t.Helper()
...
func TestRunnerPGUniqueKeyRaceOneWins(t *testing.T) {
	...
	for range goroutines {
		go func() {
			defer wg.Done()
			_, err := Insert(context.Background(), NewClient(db),
				pgConcurrentArgs{V: "race"}, InsertOpts{UniqueKey: "racing"})
			switch {
			case err == nil:
				success.Add(1)
			case errorsIs(err, ErrAlreadyEnqueued):
				conflict.Add(1)
			default:
...
func TestRunnerPGLeaseExpiryReclaimsJob(t *testing.T) {
	...
	id, err := Insert(context.Background(), NewClient(db),
		pgConcurrentArgs{V: "stuck"}, InsertOpts{})
	require.NoError(t, err)
...
func (w *pgFanOutWorker) Work(_ context.Context, job *Job[pgFanOutArgs]) (Result, error) {
	w.processed.Add(1)
	followUps := make([]FollowUp, job.Args.Children)
...
func (w *pgChildWorker) Work(_ context.Context, job *Job[pgChildArgs]) (Result, error) {
	w.processed.Add(1)
...
func TestRunnerPGFanOutAtomicityNoDoubleDispatch(t *testing.T) {
	...
	_, err := Insert(context.Background(), NewClient(db),
		pgFanOutArgs{Children: children}, InsertOpts{})
	require.NoError(t, err)
